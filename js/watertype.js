$( document ).ready(function() {
  ...
  var QUALITY_X = 2, QUALITY_Y = 1,
    WIDTH = Math.floor(window.innerWidth / QUALITY_X),
    HEIGHT = Math.floor(window.innerHeight / QUALITY_Y), SIZE = WIDTH * HEIGHT,
...
function init() {
  ...
  input.type = "text";
  input.value = "ynunokawa.github.io";
  ...
  canvasHeightMap.class = "coverwater";
...
function emit(x, y) {
  ...
  buffer1[Math.floor(x) + (Math.floor(y + 64) * WIDTH)] = 256;
...
function writeText(string) {
  ...
  contextText.fillText(string, WIDTH / 2, 96);
